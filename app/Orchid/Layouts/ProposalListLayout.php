...
class ProposalListLayout extends Table
{
    ...
    {
        return [
            TD::make('title', 'Title')->render(function (Proposal $proposal) {
            ...
            TD::make('created_at', 'Created')->render(
                fn(Proposal $proposal) => $this->formatDate($proposal->created_at),
            ),
            TD::make('updated_at', 'Last edit')->render(
                fn(Proposal $proposal) => $this->formatDate($proposal->updated_at),
            ),
            TD::make()->render(
                fn(Proposal $proposal) => Group::make([
                    Link::make('View')
                        ->icon('eye')
                        ->target('_blank')
